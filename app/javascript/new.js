...
if (isInView) {
  ...
  const houseworks = ["clean", "cook"];
houseworks.forEach((housework, index)=>{
var ctx=document.querySelectorAll(".chart")[index];//グラフを描画したい場所のid
const wifes = document.querySelectorAll(`.wife.${housework}`);
const husbands = document.querySelectorAll(`.husband.${housework}`);
let wpercents = []
let hpercents = []
wifes.forEach((wife)=>{
